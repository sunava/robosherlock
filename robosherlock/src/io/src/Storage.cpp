...
void Storage::loadView(uima::CAS& cas, const mongo::BSONElement& elem)
{
  ...
  try
  {
    ...
    view = cas.getView(icu_66::UnicodeString::fromUTF8(viewName));
  }
  catch (...)
  {
    outDebug("create view " << viewName);
    view = cas.createView(icu_66::UnicodeString::fromUTF8(viewName));
  }
  ...
  view->setSofaDataArray(fs, icu_66::UnicodeString::fromUTF8(mime));
}
...
void Storage::storeCollection(uima::CAS& cas, const std::string& view, const std::string& collection, int cam_id)
{
  ...
  try
  {
    uima::CAS* _view = cas.getView(icu_66::UnicodeString::fromUTF8(view_name.str()));
...
void Storage::loadCollection(uima::CAS& cas, const std::string& view, const std::string& collection, int cam_id)
{
  ...
  try
  {
    ...
    _view = cas.getView(icu_66::UnicodeString::fromUTF8(view_name.str()));
  }
  catch (...)
  {
    outDebug("create view " << view);
    _view = cas.createView(icu_66::UnicodeString::fromUTF8(view_name.str()));
  }
  ...
  _view->setSofaDataArray(fs, icu_66::UnicodeString::fromUTF8(mime));
}
